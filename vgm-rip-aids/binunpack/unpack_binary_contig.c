...
void unpackBinaryFile(FILE *in_file, char *out_dirname, int cur_entry)
{
   ...
   sprintf(out_fname, "%s[%d]", out_dirname, cur_entry);
   ...
   /*Contiguous*/
   if(entry_offset==0) return;
   entry_size -= entry_offset;
   entry_size = (entry_size <= 0) ? 0xffff : entry_size;
